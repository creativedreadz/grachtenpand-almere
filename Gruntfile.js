module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		config: {
			...
			fontPathSrc: '<%= config.src %>/font',
			fontPathDest: '<%= config.dist %>/font'
		},
		...
		watch: {
			...
			img: {
				files: '<%= config.imgPathSrc %>/**/*.{png,jpg,svg}',
				tasks: ['copy:img']
			},
			font: {
				files: '<%= config.fontPathSrc %>/*.{eot,ttf,woff}',
				tasks: ['copy:font']
			}
		},
		...
		copy: {
			...
			font: {
				files: [{
					expand: true,
					cwd: '<%= config.fontPathSrc %>',
					src: ['**/*'],
					dest: '<%= config.fontPathDest %>'
				}]
			}
		},
